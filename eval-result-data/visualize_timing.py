...
def create_timing_bar_chart(cases, durations, output_path, title_suffix="", highlight_cases=None):
    ...
    colors = ['red' if highlight_cases and c in highlight_cases else 'steelblue' for c in cases]
    
    bars = ax.bar(x, durations, color=colors, alpha=0.7, edgecolor='black', linewidth=0.5)
    ...
    ax.set_title(f'Execution Time by Case{title_suffix}', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
...
def create_timing_distribution(durations, output_path, title_suffix=""):
    ...
    # Histogram
    ax1.hist(durations, bins=20, color='steelblue', alpha=0.7, edgecolor='black')
    ax1.axvline(np.mean(durations), color='red', linestyle='--', linewidth=2, label=f'Mean: {np.mean(durations):.1f}s')
    ax1.axvline(np.median(durations), color='green', linestyle='--', linewidth=2, label=f'Median: {np.median(durations):.1f}s')
    ax1.set_xlabel('Execution Time (seconds)', fontsize=12, fontweight='bold')
    ax1.set_ylabel('Frequency', fontsize=12, fontweight='bold')
    ax1.set_title(f'Distribution of Execution Times{title_suffix}', fontsize=13, fontweight='bold')
    ax1.legend()
    ...
    ax2.set_title(f'Execution Time Statistics{title_suffix}', fontsize=13, fontweight='bold')
...
def create_cumulative_time_chart(cases, durations, output_path, title_suffix=""):
    ...
    ax.plot(cases, cumulative, 'o-', linewidth=2, markersize=6, color='steelblue', alpha=0.7)
    ...
    ax.set_title(f'Cumulative Execution Time{title_suffix}', fontsize=14, fontweight='bold')
...
def create_comparison_chart(data1_label, cases1, durations1, data2_label, cases2, durations2, output_path):
    ...
    ax1.set_title('Execution Time Comparison by Case', fontsize=13, fontweight='bold')
    ax1.set_xticks(x)
    ax1.set_xticklabels([f'C{c}' for c in common_cases], rotation=45, ha='right')
    ax1.legend()
    ax1.grid(axis='y', alpha=0.3, linestyle='--')
    ...
    ax2.set_ylabel('Execution Time (seconds)', fontsize=12, fontweight='bold')
    ax2.set_title('Execution Time Distribution Comparison', fontsize=13, fontweight='bold')
    ax2.grid(axis='y', alpha=0.3, linestyle='--')
...
def create_timing_vs_accuracy_chart(cases, durations, f1_scores, output_path, title_suffix=""):
    ...
    for case, dur, f1 in zip(cases, durations, f1_scores):
        ax.annotate(f'C{case}', (dur, f1), fontsize=8, ha='center', va='center')
    
    ax.set_xlabel('Execution Time (seconds)', fontsize=12, fontweight='bold')
    ax.set_ylabel('F1 Score', fontsize=12, fontweight='bold')
    ax.set_title(f'Execution Time vs F1 Score{title_suffix}', fontsize=14, fontweight='bold')
    ax.grid(True, alpha=0.3, linestyle='--')
...
def main():
    ...
    comparison_file1 = base_dir / "comparison_all_cases_benchmark_vertica_20251204_094407.json"
    comparison_file2 = base_dir / "comparison_all_cases_benchmark_vertica_20251204_111450.json"
    ...
    print(f"  Total Time: {np.sum(all_durations):.1f}s ({np.sum(all_durations)/3600:.2f}h)")
    print(f"  Average Time: {np.mean(all_durations):.1f}s (±{np.std(all_durations):.1f}s)")
    print(f"  Median Time: {np.median(all_durations):.1f}s")
    print(f"  Min Time: {np.min(all_durations):.1f}s (Case {all_cases[np.argmin(all_durations)]})")
    print(f"  Max Time: {np.max(all_durations):.1f}s (Case {all_cases[np.argmax(all_durations)]})")
    
    print(f"\nCases 1-12 ({len(cases2)} cases):")
    print(f"  Total Time: {np.sum(durations2):.1f}s ({np.sum(durations2)/3600:.2f}h)")
    print(f"  Average Time: {np.mean(durations2):.1f}s (±{np.std(durations2):.1f}s)")
    print(f"  Median Time: {np.median(durations2):.1f}s")
    
    print(f"\nCases 14-50 ({len(cases1)} cases):")
    print(f"  Total Time: {np.sum(durations1):.1f}s ({np.sum(durations1)/3600:.2f}h)")
    print(f"  Average Time: {np.mean(durations1):.1f}s (±{np.std(durations1):.1f}s)")
