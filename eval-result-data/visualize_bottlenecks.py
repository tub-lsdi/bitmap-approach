...
def extract_detailed_timings(benchmark_data):
    ...
    for result in benchmark_data.get('results', []):
        ...
        vertica_connection = go_timings.get('vertica_connection', {}).get('duration_seconds', 0)
        load_bitmaps = go_timings.get('load_and_create_bitmaps', {}).get('duration_seconds', 0)
        filter_bitmaps = go_timings.get('filter_bitmaps_to_relevant_tables', {}).get('duration_seconds', 0)
        pair_counts = go_timings.get('calculate_pair_table_counts', {}).get('duration_seconds', 0)
        quad_scores = go_timings.get('calculate_quad_scores', {}).get('duration_seconds', 0)
        total_count = go_timings.get('get_total_table_count', {}).get('duration_seconds', 0)
        pmi_scores = go_timings.get('calculate_pmi_scores', {}).get('duration_seconds', 0)
...
def create_stacked_bar_chart(case_timings, output_path):
    ...
    go_calc = np.array([ct['pair_counts'] + ct['quad_scores'] + ct['pmi_scores'] for ct in case_timings])
    
    # PYTHON SERVICE operations
    python_ops = np.array([ct['solve_cilp'] + ct['extract_join'] + ct['convert_output'] for ct in case_timings])
    
    # OTHER (connection setup)
    other = np.array([ct['other'] + ct['vertica_connection'] for ct in case_timings])
    ...
    p6 = ax.bar(x, other, width, 
                bottom=vertica_query_load + vertica_total_count + go_filter + go_calc + python_ops,
    ...
    ax.set_title('Time Breakdown by Operation - All Cases', fontsize=15, fontweight='bold')
    ax.set_xticks(x)
    ax.set_xticklabels([f'C{c}' for c in cases], rotation=45, ha='right', fontsize=9)
...
def create_average_breakdown_pie(case_timings, output_path):
    ...
    avg_calc = np.mean([ct['pair_counts'] + ct['quad_scores'] + ct['pmi_scores'] for ct in case_timings])
    avg_python = np.mean([ct['solve_cilp'] + ct['extract_join'] + ct['convert_output'] for ct in case_timings])
    avg_other = np.mean([ct['other'] + ct['vertica_connection'] for ct in case_timings])
    
    total_avg = avg_load_bitmaps + avg_total_count + avg_filter + avg_calc + avg_python + avg_other
    ...
    sizes = [avg_load_bitmaps, avg_total_count, avg_filter, avg_calc, avg_python, avg_other]
    ...
    labels2 = ['Data Loading\n(mixed: DB scan + network + Go)', 'Vertica Queries', 'Go Processing', 'Python', 'Other']
...
def create_bottleneck_analysis(case_timings, output_path):
    ...
    total_other = sum([ct['other'] + ct['vertica_connection'] for ct in case_timings])
    ...
    bars = ax.barh(y_pos, times, color=colors, alpha=0.8, edgecolor='black', linewidth=0.5)
    ...
    ax.set_xlabel('Total Time Across All Cases (seconds)', fontsize=12, fontweight='bold')
    ax.set_title('Bottleneck Analysis: Total Time by Operation', fontsize=14, fontweight='bold')
    ax.grid(axis='x', alpha=0.3, linestyle='--')
    
    # Add legend
    from matplotlib.patches import Patch
    legend_elements = [
        Patch(facecolor='#d62728', alpha=0.8, label='Data Loading & DB Queries (mixed)'),
...
def create_vertica_operations_detail(case_timings, output_path):
    ...
    ax.bar(x - 2.5*width, load_bitmaps, width, label='Data Loading (mixed)', color='#d62728', alpha=0.8)
    ax.bar(x - 1.5*width, total_count, width, label='COUNT Query', color='#ff7f0e', alpha=0.8)
    ax.bar(x - 0.5*width, filter_bitmaps, width, label='Filter Bitmaps', color='#1f77b4', alpha=0.8)
    ax.bar(x + 0.5*width, pair_counts, width, label='Pair Counts', color='#17becf', alpha=0.8)
    ax.bar(x + 1.5*width, quad_scores, width, label='Quad Scores', color='#9edae5', alpha=0.8)
    ax.bar(x + 2.5*width, pmi_scores, width, label='PMI Scores', color='#aec7e8', alpha=0.8)
    
    ax.set_xlabel('Case Number', fontsize=12, fontweight='bold')
    ax.set_ylabel('Execution Time (seconds)', fontsize=12, fontweight='bold')
    ax.set_title('Go Service Operations Breakdown by Case', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
    ax.set_xticklabels([f'C{c}' for c in cases], rotation=45, ha='right', fontsize=9)
...
def main():
    ...
    avg_calc = np.mean([ct['pair_counts'] + ct['quad_scores'] + ct['pmi_scores'] for ct in all_timings])
    avg_python = np.mean([ct['solve_cilp'] + ct['extract_join'] + ct['convert_output'] for ct in all_timings])
    avg_other = np.mean([ct['other'] + ct['vertica_connection'] for ct in all_timings])
    avg_total = avg_load + avg_total_count + avg_filter + avg_calc + avg_python + avg_other
    ...
    print(f"   - Filter Bitmaps: {avg_filter:.1f}s ({avg_filter/avg_total*100:.1f}%)")
    print(f"   - Calculations (pair/quad/PMI): {avg_calc:.1f}s ({avg_calc/avg_total*100:.1f}%)")
    ...
    print("="*80)
    print(f"The 'Data Loading' operation ({data_loading:.1f}s, {data_loading/avg_total*100:.1f}%) is likely dominated by:")
    print(f"  • Vertica table scan (probably 70-80% of the {data_loading:.1f}s)")
